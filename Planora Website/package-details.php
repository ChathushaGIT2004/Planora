...
if (isset($_POST['save_edit'])) {
    ...
    $stmt = $conn->prepare("UPDATE Service_Package SET package_name=?, description=?, package_price=? WHERE package_id=? AND vendor_id=?");
    $stmt->bind_param("ssiii", $new_name, $desc, $package_price, $package_id, $vendor['vendor_id']);
    $stmt->execute();
    $stmt->close();
        ...
        <div class="details-section">
            <div id="view-mode">
                ...
                <?php if (!empty($package['price_min']) && !empty($package['price_max']) && $package['price_min'] != $package['price_max']): ?>
                    <div class="package-price">LKR <?php echo number_format($package['price_min']); ?> - <?php echo number_format($package['price_max']); ?></div>
                <?php else: ?>
                    <div class="package-price">LKR <?php echo number_format($package['package_price']); ?></div>
                <?php endif; ?>
            ...
            <form id="edit-form" class="edit-form" method="POST" enctype="multipart/form-data" style="display:none; margin-top:2rem;">
                ...
                <div style="display:flex;gap:1rem;">
                    <input type="number" name="price_min" min="0" placeholder="Min" value="<?php echo htmlspecialchars($package['price_min'] ?? $package['package_price']); ?>">
                    <input type="number" name="price_max" min="0" placeholder="Max" value="<?php echo htmlspecialchars($package['price_max'] ?? $package['package_price']); ?>">
                </div>
